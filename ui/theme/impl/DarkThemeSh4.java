...
public class DarkThemeSh4 extends ShellTheme {
	...
	public void draw(int mouseX, int mouseY, float partialTicks) {
		...
		// ==============================
            ...
            int substractY = 0;
            if (totalHeight > shellHeight - DRAGBAR_HEIGHT - 5) {
            	substractY = totalHeight - (shellHeight - DRAGBAR_HEIGHT - 5);
			}


            int y = shellY + DRAGBAR_HEIGHT + 5 - substractY	;
			...
			for (Tuple<TextBlock, Boolean> textBlockVal : Shell._self.outputs()) {
				...
				for (String line : lines) {
					if (y >= shellY + DRAGBAR_HEIGHT + 3 && y <= shellY + shellHeight)
